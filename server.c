...
/* Process and HTTP "GET" request for PAGE, and send the results to the
...
static void handle_connection (int connection_fd)
{
  ...
  if (bytes_read > 0) {
    ...
    /* The first line the client sends is the HTTP request, which is
      composed of a method, the requested pagem and the protocol version. */
    ...
    if (strcmp (protocol, "HTTP/1.0") && strcmp (protocol, "HTTP/1.1")) {
      /* We don't understand this protocol. Reporte a bad response. */
      write (connection_fd, bad_request_response, sizeof (bad_request_response));
    }
    else if (strcmp (method, "GET")) {
      /* This server only implements the GET methodl. The client
...
void server_run (struct in_addr local_address, uint16_t port)
{
  ...
  while (1) {
    ...
    if (child_pid == 0) {
      ...
      /* This is a parent process. The child process handles the
