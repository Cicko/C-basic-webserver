...
int main (int argc, char* const argv[])
{
  ...
  verbose = 0;
  /* Locad modules from the directory containing this executable. */
  ...
  do {
    ...
    switch (next_option) {
      ...
      case 'p':
        /* User specified -p or --port. */
        {
          ...
          if (*end != '\0')
            ...
            print_usage (1);
            /* The por number needs to be converted to network (big endian)
              byte order. */
      ...
      case '?':
        /* User speficied an unrecognized option. */
        print_usage (1);
    ...
    /* his program takes no additional arguments. Issue an error if the
      user specified any */
    if (optind != argc)
      print_usage (1);
